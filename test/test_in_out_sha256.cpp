...
const string filename = macro_xstr(EMP_CIRCUIT_PATH) + string("bristol_format/sha-256-big.txt");
...
void test_non_in_out(int party, int port) {
	...
	// AES: 128 128 128
	// SHA: 512 0   256
	// cout << party << "total n1: " << cf.n1 << endl;
	// cout << party << "total n2: " << cf.n2 << endl;
	// if(party == ALICE) {
...
void test_in_out(int party, int port) {
	cout << party << "FlexIn/FlexOut" << endl;
	cout << party << "compute: K = E_{010101...}(101010...); E_{010101...}(K)" << endl;
	PRG prg;
	bool delta[128];
	prg.random_bool(delta, 128);	
