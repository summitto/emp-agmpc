...
void sha256(block *input, block *output, int input_len) {
	...
	for (int b = 0; b < num_blocks; b++) {
		...
		string inputFile = inputFolder + "sha256/sha-256-multiblock-aligned.txt";
		BristolFormat bf(inputFile.c_str());
...
void DeriveHandshakeSecret_PreMasterSecret() {
	setup_plain_prot(false, "DeriveHandshakeSecret_PreMasterSecret.txt");
...
void DeriveMasterSecret() {
	setup_plain_prot(false, "DeriveMasterSecret.txt");
...
void DeriveClientHandshakeSecret() {
	setup_plain_prot(false, "DeriveClientHandshakeSecret.txt");
...
void DeriveServerHandshakeSecret() {
	setup_plain_prot(false, "DeriveServerHandshakeSecret.txt");
...
void DeriveClientHandshakeKey() {
	setup_plain_prot(false, "DeriveClientHandshakeKey.txt");
...
void DeriveClientHandshakeIV() {
	setup_plain_prot(false, "DeriveClientHandshakeIV.txt");
...
void DeriveClientTrafficSecret() {
	setup_plain_prot(false, "DeriveClientTrafficSecret.txt");
...
void DeriveClientTrafficKey() {
	setup_plain_prot(false, "DeriveClientTrafficKey.txt");
...
void DeriveClientTrafficIV() {
	setup_plain_prot(false, "DeriveClientTrafficIV.txt");
...
void CreateGCMSequence(){
	for(int BLOCK = 1; BLOCK <= 10; BLOCK ++) {
		setup_plain_prot(false, "gcm_shares_" + std::to_string(BLOCK) + ".txt");
		...
		string inputFile = inputFolder + "aes/aes128_full.txt";
		BristolFormat bf(inputFile.c_str());
...
void DeriveHandshakeSecret_PreMasterSecret_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(256, 0, emp::ALICE);
	...
	block shared_secret[256];
	for (int i = 0; i < 32; i++) {
		int w = test_shared_secret[i];
		for (int j = 0; j < 8; j++) {
			shared_secret[i * 8 + j] = w & 1 ? one : zero;
			w >>= 1;
			in1.bits[i * 8 + j].bit = shared_secret[i * 8 + j];
		}
	}

	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(256, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveHandshakeSecret_PreMasterSecret.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
	// Expected output:
	// hex(int(""[::-1],2))
	// '0x9a14df5e1520716e4a3094a7885570201b1b9af63b24332cd0a5b38906c89ffb'
}

void	DeriveMasterSecret_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(256, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(256, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveMasterSecret.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	DeriveClientHandshakeSecret_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(512, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(256, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveClientHandshakeSecret.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	DeriveServerHandshakeSecret_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(512, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(256, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveServerHandshakeSecret.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	DeriveClientHandshakeKey_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(256, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(128, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveClientHandshakeKey.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	DeriveClientHandshakeIV_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(256, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(96, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveClientHandshakeIV.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	DeriveClientTrafficSecret_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(256, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(256, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveClientTrafficSecret.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	DeriveClientTrafficKey_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(256, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(128, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveClientTrafficKey.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	DeriveClientTrafficIV_plain() {
	emp::setup_plain_prot(false, "tmp.txt");
	emp::Integer in1(256, 0, emp::ALICE);
	emp::Integer in2(0, 0, emp::BOB);
	emp::Integer out(96, 0, emp::PUBLIC);
	std::string filepath = inputFolder + "key-derivation/DeriveClientTrafficIV.txt";
	emp::BristolFormat cf(filepath.c_str());
	cf.compute(out.bits.data(), in1.bits.data(), in2.bits.data());
	std::cout << out.reveal<std::string>() << std::endl;
	emp::finalize_plain_prot();
...
void	RunAllDerivations_plain() {
	emp::setup_plain_prot(false, "tmp.txt");

	block one = CircuitExecution::circ_exec->public_label(true);
	block zero = CircuitExecution::circ_exec->public_label(false);

	emp::Integer in0(0, 0, emp::BOB);
	emp::Integer in1(256, 0, emp::ALICE);
	...
	emp::Integer out1(256, 0, emp::PUBLIC);
	std::string filepath1 = inputFolder + "key-derivation/DeriveHandshakeSecret_PreMasterSecret.txt";
	emp::BristolFormat cf1(filepath1.c_str());
	cf1.compute(out1.bits.data(), in1.bits.data(), in0.bits.data());
	std::string res1 = out1.reveal<std::string>();
	std::reverse(res1.begin(), res1.end());	
	std::stringstream reader_reverse(res1);
	...
	std::cout << "out1: " << result_reverse.str() << endl;
	// expected: out1: 4cc8c75efedbbc89de77a14844639936d4491174f7cb12166b96f5a22597b250

	emp::Integer in2(512, 0, emp::ALICE);
	for (auto i = 0; i < 256; i++) {
		in2.bits[i].bit = out1.bits[i].bit;
	}
	...
	block hello_hash[256];
	for (int i = 0; i < 32; i++) {
		int w = test_hello_hash[i];
		for (int j = 0; j < 8; j++) {
			hello_hash[i * 8 + j] = w & 1 ? one : zero;
			w >>= 1;
			in2.bits[256 + i * 8 + j].bit = hello_hash[i * 8 + j];
		}
	}

	emp::Integer out2(256, 0, emp::PUBLIC);
	std::string filepath2 = inputFolder + "key-derivation/DeriveClientHandshakeSecret.txt";
	emp::BristolFormat cf2(filepath2.c_str());
	cf2.compute(out2.bits.data(), in2.bits.data(), in0.bits.data());
	std::string res2 = out2.reveal<std::string>();
	std::reverse(res2.begin(), res2.end());	
	std::stringstream reader_reverse2(res2);
	std::stringstream result_reverse2;
	while (reader_reverse2) {
		std::bitset<8> digit;
		reader_reverse2 >> digit;
		result_reverse2 << std::hex << digit.to_ulong();
	}
	std::cout << "out2: " << result_reverse2.str() << endl; // CLIENT_HANDSHAKE_TRAFFIC_SECRET
	// expected: out2: 6a4234abaf344662966da74f5fd94ea3c25940645aa8629733341353631f650

	emp::Integer out3(256, 0, emp::PUBLIC);
	std::string filepath3 = inputFolder + "key-derivation/DeriveServerHandshakeSecret.txt";
	emp::BristolFormat cf3(filepath3.c_str());
	cf3.compute(out3.bits.data(), in2.bits.data(), in0.bits.data());
	std::cout << "out3: " << out3.reveal<std::string>() << std::endl;

	emp::Integer out4(128, 0, emp::PUBLIC);
	std::string filepath4 = inputFolder + "key-derivation/DeriveClientHandshakeKey.txt";
	emp::BristolFormat cf4(filepath4.c_str());
	cf4.compute(out4.bits.data(), out2.bits.data(), in0.bits.data());
	std::cout << "out4: " << out4.reveal<std::string>() << std::endl;

	emp::Integer out5(96, 0, emp::PUBLIC);
	std::string filepath5 = inputFolder + "key-derivation/DeriveClientHandshakeIV.txt";
	emp::BristolFormat cf5(filepath5.c_str());
	cf5.compute(out5.bits.data(), out2.bits.data(), in0.bits.data());
	std::cout << "out5: " << out5.reveal<std::string>() << std::endl;

	emp::Integer out6(256, 0, emp::PUBLIC);
	std::string filepath6 = inputFolder + "key-derivation/DeriveMasterSecret.txt";
	emp::BristolFormat cf6(filepath6.c_str());
	cf6.compute(out6.bits.data(), out1.bits.data(), in0.bits.data());
	std::cout << "out6: " << out6.reveal<std::string>() << std::endl;

	emp::Integer in7(512, 0, emp::ALICE);
	for (auto i = 0; i < 256; i++) {
		in7.bits[i].bit = out6.bits[i].bit;
	}
	...
	block finished_hash[256];
	for (int i = 0; i < 32; i++) {
		int w = test_finished_hash[i];
		for (int j = 0; j < 8; j++) {
			finished_hash[i * 8 + j] = w & 1 ? one : zero;
			w >>= 1;
			in7.bits[256 + i * 8 + j].bit = finished_hash[i * 8 + j];
		}
	}

	emp::Integer out7(256, 0, emp::PUBLIC);
	std::string filepath7 = inputFolder + "key-derivation/DeriveClientTrafficSecret.txt";
	emp::BristolFormat cf7(filepath7.c_str());
	cf7.compute(out7.bits.data(), in7.bits.data(), in0.bits.data());
	std::string res7 = out7.reveal<std::string>();
	std::reverse(res7.begin(), res7.end());	
	std::stringstream reader_reverse7(res7);
	std::stringstream result_reverse7;
	while (reader_reverse7) {
		std::bitset<8> digit;
		reader_reverse7 >> digit;
		result_reverse7 << std::hex << digit.to_ulong();
	}
	std::cout << "out7: " << result_reverse7.str() << endl;
	// expected: ee23f537c6949bd1e966ce7376a2141fcd5e36d21fe8936f87383c3982ab8ef0

	emp::Integer out8(128, 0, emp::PUBLIC);
	std::string filepath8 = inputFolder + "key-derivation/DeriveClientTrafficKey.txt";
	emp::BristolFormat cf8(filepath8.c_str());
	cf8.compute(out8.bits.data(), out7.bits.data(), in0.bits.data());
	std::cout << "out8: " << out8.reveal<std::string>() << std::endl;

	emp::Integer out9(96, 0, emp::PUBLIC);
	std::string filepath9 = inputFolder + "key-derivation/DeriveClientTrafficIV.txt";
	emp::BristolFormat cf9(filepath9.c_str());
	cf9.compute(out9.bits.data(), out7.bits.data(), in0.bits.data());
	std::cout << "out9: " << out9.reveal<std::string>() << std::endl;
...
int main(int argc, char **argv) {
	...
	DeriveHandshakeSecret_PreMasterSecret_plain();
	DeriveClientHandshakeSecret_plain();
	DeriveServerHandshakeSecret_plain();
	DeriveClientHandshakeKey_plain();
	DeriveClientHandshakeIV_plain();
	DeriveMasterSecret_plain();
	DeriveClientTrafficSecret_plain();
	DeriveClientTrafficKey_plain();
	DeriveClientTrafficIV_plain();
	
	std::cout << "-----------------------" << std::endl;

	RunAllDerivations_plain();
